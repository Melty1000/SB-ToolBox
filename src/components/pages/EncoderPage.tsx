...
import { useSBEncoder, SBActionInfo, type SBExportData } from '@/hooks/useSBEncoder';
import { SBEditor } from '@/components/editor/SBEditor';
import { useSBHistory } from '@/hooks/useSBHistory';
import { cn } from '@/lib/utils';
import {
    ChevronDown, FolderCode,
    Zap, Trash2, CheckCircle2, AlertCircle, FileUp,
    Terminal, Braces, Code2, Search, Loader2, Copy
} from 'lucide-react';
...
export function EncoderPage({ initialValue }: { initialValue?: string }) {
    const { encode, error, availableActions, parseActions, decode } = useSBEncoder();
    ...
    const editorPaneHeightClass = "h-[clamp(340px,calc(100vh-300px),760px)]";

    const isRecord = (value: unknown): value is Record<string, unknown> =>
        typeof value === 'object' && value !== null;

    const getList = (parent: unknown, ...keys: string[]) => {
    ...
    const safeStr = (value: unknown, fallback: string) =>
        (typeof value === 'string' && value !== 'undefined' && value !== '') ? value : fallback;
    ...
    const loadTemplateContent = (rawContent: string, fileName?: string) => {
        const content = String(rawContent || '');
        const trimmed = content.trim();
        const lowerName = (fileName || '').toLowerCase();
        const isSbFile = lowerName.endsWith('.sb');
        ...
        const tryLoadJson = () => {
            try {
                ...
                setJsonTemplate(JSON.stringify(parsed, null, 2));
        ...
        const tryLoadSb = () => {
            ...
            if (decoded?.data) {
                setJsonTemplate(JSON.stringify(decoded.data, null, 2));
                showToast("SB Template Decoded");
                return true;
            }
            return false;
        };

        if (isSbFile || trimmed.startsWith('SBAE')) {
            if (tryLoadSb()) return;
            if (tryLoadJson()) return;
            showToast("Unable to decode SB template", "error");
            return;
        }

        if (tryLoadJson()) return;
        if (tryLoadSb()) return;
        showToast("Unsupported template format", "error");
    };
    ...
    const handleJsonDrop = (e: React.DragEvent) => {
        ...
        const templateFiles = files.filter((f) => {
            ...
            return lower.endsWith('.json') || lower.endsWith('.sb') || lower.endsWith('.txt');
        });

        if (templateFiles.length > 0) {
            const targetFile = templateFiles[0];
            const reader = new FileReader();
            reader.onload = (e) => {
                const content = e.target?.result as string;
                loadTemplateContent(content, targetFile.name);
            };
        ...
        try {
            ...
            Object.keys(scripts || {}).forEach(fileName => {
                if (!newMap[fileName]) {
                    ...
                    const match = availableActions.find((a) => {
    ...
    const handleEncode = (mode: 'file' | 'string') => {
        ...
        safeTimeout(() => {
            try {
                const data = JSON.parse(jsonTemplate) as SBExportData;
                const encoded = encode(data, scripts || {}, linkMap || {});
                if (encoded) {
                    setResult(encoded);

                    // Add to history
                    const exportRoot = isRecord(data) ? (data.Export ?? data.export ?? data) : data;
                    const meta = isRecord(exportRoot) ? (exportRoot.meta ?? exportRoot) : exportRoot;

                    addToHistory({
                        type: 'encode',
                        name: safeStr(isRecord(meta) ? (meta.Name ?? meta.name) : undefined, 'Untitled Export'),
                        author: safeStr(isRecord(meta) ? (meta.Author ?? meta.author) : undefined, 'Unknown'),
                        version: safeStr(isRecord(meta) ? (meta.Version ?? meta.version) : undefined, '0.0.0'),
                        rawString: encoded,
                        stats: {
                ...
                showToast(e instanceof Error ? e.message : "Encoding failed", 'error');
            } finally {
    ...
    const handleDownloadSB = (content: string) => {
        try {
            ...
            const rawName = String(meta.Name || meta.name || data.Name || data.name || 'melt_export').trim();
            const safeName = rawName.replace(/[<>:"/\\|?*\u0000-\u001F]/g, '_') || 'melt_export';
            const fileName = `${safeName}.sb`;
        ...
        <div className="flex flex-col gap-8 pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                <div className="flex flex-col gap-4">
                    ...
                    <div
                        className={cn(
                            "relative group transition-all duration-300 border-b border-transparent hover:border-b-melt-accent/50 focus-within:border-b-melt-accent/50",
                            editorPaneHeightClass,
                ...
                <div className="flex flex-col gap-4 h-full">
                    ...
                    <div
                        ...
                        className={cn(
                            "relative group transition-all duration-700 ease-in-out flex flex-col border-b border-transparent hover:border-b-melt-accent/50 overflow-y-auto custom-scrollbar",
                            editorPaneHeightClass,
