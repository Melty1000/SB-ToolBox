...
import { getElectronApi, isElectronRuntime, type AppSettings } from '@/lib/electron-api';
...
interface ShellConfig extends Omit<AppSettings, 'theme'> {
    theme: ThemeVariant;
}
...
export const MeltShell: React.FC<MeltShellProps> = ({ children, activePage = 'decoder', setActivePage }) => {
    ...
    useEffect(() => {
        ...
        const loadSettings = async () => {
            const { settings, selection } = await bootstrapThemeSelection(getElectronApi());
            if (disposed) return;
    ...
    useEffect(() => {
        ...
        if (electron?.updater) {
            const cleanupAvailable = electron.updater.on('update:available', () => {
                setHasUpdate(true);
            });
            const cleanupReady = electron.updater.on('update:ready', () => {
                setUpdateReady(true);
                setHasUpdate(true);
            });
        ...
        >
            ...
            <aside
                ...
                onBlurCapture={(event) => {
                    ...
                    if (nextTarget && sidebarRef.current?.contains(nextTarget)) return;
                    collapseSidebar();
                }}
            >
                <div
                    className={cn(
                        "h-[72px] w-full mb-0 flex items-center justify-center",
                        isDesktop && "drag-region no-drag"
                    )}
                >
                    <button
                        type="button"
                        aria-label="Go to Decoder"
                    ...
                    >
                        <div className="logo-container h-12 w-full relative grid place-items-center overflow-visible">
                            <img
                                src={withBasePath('/assets/logo-collapsed.svg')}
                                alt="Mascot"
                            ...
                            />
                            <img
                                src={withBasePath('/assets/streamerbot-logo.svg')}
                                alt="Links"
...
const NavItem = ({ icon: Icon, label, active, isExpanded, onClick, id, hasBadge }: NavItemProps) => (
    <button
        ...
        id={`nav-item-${id}`}
        className={cn(
            "relative z-10 pointer-events-auto flex items-center justify-center rounded-md group w-full h-10 nav-item overflow-hidden",
...
const ControlBtn = ({ icon: Icon, onClick, isClose = false }: ControlBtnProps) => (
    <button
        onClick={onClick}
        className={cn(
