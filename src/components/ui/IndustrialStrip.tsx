...
import { ChevronDown, Code2, type LucideIcon } from 'lucide-react';
...
export const IndustrialStrip = ({
    ...
    const [isContentMounted, setIsContentMounted] = useState(() => !lazyContent || isExpanded);
    ...
    React.useEffect(() => {
        if (isExpanded && lazyContent) {
            setIsContentMounted(true);
        }
    }, [isExpanded, lazyContent]);
    ...
    useGSAP(() => {
        ...
        if (isExpanded) {
            ...
            gsap.to(node, {
                ...
                overwrite: 'auto',
                onComplete: () => {
                    if (lazyContent && !isExpandedRef.current && isMountedRef.current) {
                        setIsContentMounted(false);
                    }
                }
            });
        }
    }, { dependencies: [isExpanded, lazyContent, isContentMounted], scope: containerRef });
