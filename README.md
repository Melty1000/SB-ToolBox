...
Import strings are processed in six stages: the raw `.sb` string is validated against the binary `SBAE` header (`0x53 0x42 0x41 0x45`), the header is stripped, the remaining payload is Base64 URL-decoded, decompressed via GZip, parsed as JSON, and then recursively walked to extract any embedded C# sub-action scripts.
...
- Web build appears stale:
  hard refresh (`Ctrl+Shift+R`) after the GitHub Pages deployment completes.
